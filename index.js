...
function getFullNames(runners) {
	fullRunners =[];
	// fullNames = [];
	// fullRunners.forEach(fullNames);
	
	fullNames = runners.forEach((runner)=> fullRunners.push(runner.last_name +', '+ runner.first_name))
	return fullRunners;
}
...
function firstNamesAllCaps(runners) {
	// fullRunners = [];
	// fullNames = runners.forEach((runner)=> fullRunners.push(runner.first_name.toUpperCase()))
	uNames = runners.map((runner) => runner.first_name.toUpperCase())
	return uNames;
}
...
function getRunnersByTShirtSize(runners,tShirtSize) {
	shirtItem = runners.filter((shirt)=> shirt.shirt_size === tShirtSize)
	return shirtItem;
}
...
function tallyUpDonations(runners) {
	// tally = runners.filter((runner)=> runner.donation !== null);
	// console.log( tally)

	// tally = runners.map((don)=> {
	// 	if (don.donation === null){
	// 		return don.donation}else{
	// 		return 0}});
	sum = runners.reduce((donAmt,don) => donAmt + don.donation,0) 
	return sum;
